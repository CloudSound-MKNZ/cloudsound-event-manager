...
class EventParser:
    ...
    BANDCAMP_PATTERN = re.compile(r"https?://[\w-]+\.bandcamp\.com/(?:track|album)/[\w-]+")
    SOUNDCLOUD_PATTERN = re.compile(r"https?://(?:www\.)?soundcloud\.com/[\w-]+/[\w-]+")
    SPOTIFY_PATTERN = re.compile(r"https?://open\.spotify\.com/(?:track|album)/[\w]+")

    # Price patterns
    PRICE_PATTERN = re.compile(
        r"(?:tickets?|entry|vstopnina)[:\s]*(\d+(?:[.,]\d{2})?)\s*€", re.IGNORECASE
    )
    ...
    def _extract_artists(self, name: str, description: str) -> List[str]:
        ...
        feat_pattern = re.compile(r"(?:feat\.|featuring|ft\.)\s+([^,\n]+)", re.IGNORECASE)
    ...
    def _validate(self, parsed: ParsedEvent) -> None:
        ...
        if parsed.start_time:
            ...
            if event_time < six_months_ago:
                errors.append("Event is too far in the past")
            # Note: We allow events in the past (up to 6 months) to sync historical data
